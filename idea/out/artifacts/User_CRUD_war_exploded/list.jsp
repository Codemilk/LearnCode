        ...
        window.onload=function(){
            ...
            document.getElementById("delSelect").onclick=function(){
                ...
                if(confirm("您确定要删除这些信息吗")){
                   //还需要判断一下是否有数据，如果没有数据会出现500空指针异常
                    var flag=false;

                    var cbs = document.getElementsByName("uid");

                    for(var i=0;i<cbs.length;i++){
                        //设置这些cbs [i]的状态等于cb的状态
                       if(cbs[i].checked){
                           flag=true;
                       }
                    }
    ...
    <div style="float: left">
        <form action="${pageContext.request.contextPath}/findUserByPageServlet" method="post" class="form-inline">
            ...
            <div class="form-group">
                ...
                <input name="address" type="text" value="${parameterMap.address[0]}" class="form-control" id="exampleInputName3">
            </div>
            <div class="form-group">
                <label for="exampleInputEmail2">email</label>
                <input name="email" type="text" value="${parameterMap.email[0]}" class="form-control" id="exampleInputEmail2">
            </div>
...
<%--                        <c:if test="${pb.curentPage==pb.totalPage}">--%>
<%--                            <li class="disabled">--%>
<%--                        </c:if>--%>
...
<%--                        </c:if>--%>
<%--                    <c:if test="${pb.currentPage>=pb.totalPage}">--%>
<%--                        ${pb.currentPage=pb.currentPage-1}--%>
<%--                    </c:if>--%>
                    <li>
